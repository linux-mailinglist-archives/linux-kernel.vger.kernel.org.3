...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id C6A0C4D10A5
	for <lists+linux-kernel@lfdr.de>; Tue,  8 Mar 2022 08:01:39 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1344390AbiCHHC0 (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 8 Mar 2022 02:02:26 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:54042 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1344265AbiCHHCX (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 8 Mar 2022 02:02:23 -0500
Received: from mail-yb1-xb2c.google.com (mail-yb1-xb2c.google.com [IPv6:2607:f8b0:4864:20::b2c])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id C6EAD3D49B;
        Mon,  7 Mar 2022 23:01:24 -0800 (PST)
Received: by mail-yb1-xb2c.google.com with SMTP id e186so35878581ybc.7;
        Mon, 07 Mar 2022 23:01:24 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20210112;
        h=mime-version:references:in-reply-to:from:date:message-id:subject:to
         :cc:content-transfer-encoding;
        bh=diRdIBf7RPSXDZPitZOVGC35axjnS58iwpFHO1azNUw=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=diRdIBf7RPSXDZPitZOVGC35axjnS58iwpFHO1azNUw=;
        b=
X-Gm-Message-State: AOAM531DdVeIACnx8Wn7jTxlO44FQ8TubnprW16ObH3KEQmw7BcGpCgz
        lH3mJTOmacQQsp5LP+0FWeTTLly8QVUpRdwyJ2E=
X-Google-Smtp-Source: ABdhPJwrmMG7mb2BtQv/W+rNV19Wa1lCZkPo6r6XfaKVSMhBIN9GSN2L67/3Me8cRYUZecsPFKgNkBRfzRnxgEwimE0=
X-Received: by 2002:a25:9f8a:0:b0:628:b9f3:6d2f with SMTP id
 u10-20020a259f8a000000b00628b9f36d2fmr10606904ybq.151.1646722883537; Mon, 07
 Mar 2022 23:01:23 -0800 (PST)
MIME-Version: 1.0
References: <20220307214303.1822590-1-max@enpas.org>
In-Reply-To: <20220307214303.1822590-1-max@enpas.org>
From:   Vincent Mailhol <vincent.mailhol@gmail.com>
Date:   Tue, 8 Mar 2022 16:01:12 +0900
Message-ID: <CAMZ6RqJZObevKPc29jW+m6i0eAgestTbw8KCPXxSGHzSXK7cRg@mail.gmail.com>
...
On Tue. 8 Mar 2022 =C3=A0 10:09, Max Staudt <max@enpas.org> wrote:
...
>  create mode 100644 Documentation/networking/device_drivers/can/elmcan.rs=
t
...
>
> +config CAN_ELMCAN
> +       tristate "Serial / USB serial ELM327 based OBD-II Interfaces (elm=
can)"
> +       depends on TTY
> +       help
> +         CAN driver for several 'low cost' OBD-II interfaces based on th=
e
> +         ELM327 OBD-II interpreter chip.
> +
> +         This is a best effort driver - the ELM327 interface was never
> +         designed to be used as a standalone CAN interface. However, it =
can
> +         still be used for simple request-response protocols (such as OB=
D II),
> +         and to monitor broadcast messages on a bus (such as in a vehicl=
e).
> +
> +         Please refer to the documentation for information on how to use=
 it:
> +         Documentation/networking/device_drivers/can/elmcan.rst
> +
...
> +       struct work_struct      tx_work;        /* Flushes TTY TX buffer =
  */
> +       unsigned char           *txbuf;
> +       unsigned char           *txhead;        /* Pointer to next TX byt=
e */
...
> +{
> +       if (elm->state !=3D ELM_GETDUMMYCHAR && elm->state !=3D ELM_GETPR=
OMPT) {
...
> +                                               ? 0
> +                                               : ELM327_CAN_CONFIG_SEND_=
SFF)
> +                                       | ELM327_CAN_CONFIG_VARIABLE_DLC
> +                                       | ELM327_CAN_CONFIG_RECV_BOTH_SFF=
_EFF
...
> +       "AT ST FF\r",     /* v1.0: Set maximum Timeout for response after=
 TX */
...
https://elixir.bootlin.com/linux/latest/source/drivers/net/can/usb/etas_es5=
8x/es58x_core.c#L815

> +       netdev_err(elm->dev, "ELM327 misbehaved. Blocking further communi=
cation.\n");
> +
...
> +               netdev_err(elm->dev,
> +                          "ELM327 reported UNABLE TO CONNECT. Please che=
ck your setup.\n");
> +       } else if (!_len_memstrcmp(elm->rxbuf, len, "BUFFER FULL")) {
> +               /* This will only happen if the last data line was comple=
te.
...
> +               netdev_err(elm->dev, "ELM327 reported an ERR%c%c. Please =
...
> +       }
> +
> +       /* Use spaces in CAN ID to distinguish 29 or 11 bit address lengt=
h.
...
> +                * The line will be re-parsed as an error line, which wil=
l fail.
> +                * However, this will correctly drop the state machine ba=
ck into
> +                * command mode.
> +                */
> +               return -ENODATA;
...
> +               frame.data[i] =3D (hex_to_bin(elm->rxbuf[datastart + 3*i]=
) << 4)
> +                             | (hex_to_bin(elm->rxbuf[datastart + 3*i + =
1]));
> +       }
> +
...
if(elm->state =3D=3D ELM_RECEIVING && elm327_parse_frame(elm, len))
...
> +               }
> +
> +       } else if (test_and_clear_bit(TODO_SILENT_MONITOR, &elm->cmds_tod=
o)) {
...
> +
> +       } else if (test_and_clear_bit(TODO_CAN_CONFIG_PART2, &elm->cmds_t=
odo)) {
> +               sprintf(local_txbuf, "ATPB%04X\r",
> +                       elm->can_config);
> +
> +       } else if (test_and_clear_bit(TODO_CANID_29BIT_HIGH, &elm->cmds_t=
odo)) {
> +               sprintf(local_txbuf, "ATCP%02X\r",
> +                       (frame->can_id & CAN_EFF_MASK) >> 24);
> +
> +       } else if (test_and_clear_bit(TODO_CANID_29BIT_LOW, &elm->cmds_to=
do)) {
...
> +                        */
> +                       netdev_err(elm->dev,
> +                                  "RX buffer overflow. Faulty ELM327 or =
UART?\n");
> +                       elm327_hw_failure(elm);
> +                       break;
...
> +                       if (elm327_is_ready_char(elm->rxbuf[elm->rxfill -=
 1])) {
> +                               /* The ELM327's AT ST response timeout ra=
n out,
> +                                * so we got a prompt.
...
> +                       }
> +
> +                       /* No <CR> found - we haven't received a full lin=
e yet.
> +                        * Wait for more data.
> +                        */
> +                       break;
...
> +       if (elm->hw_failure) {
> +               netdev_err(elm->dev, "Refusing to open interface after a =
...
> +       err =3D can_rx_offload_add_fifo(dev, &elm->offload, ELM327_NAPI_W=
EIGHT);
> +       if (err) {
> +               close_candev(dev);
> +               return err;
...
> +               elm327_send(elm, ELM327_DUMMY_STRING, 1);
> +
> +               /* Clear the wakeup bit, as the netdev will be down and t=
hus
...
Even if this check succeeds, interface might still go down at the next cycl=
e.
...
> + * This is to ensure ordering in case we are shutting down, and to ensur=
e
...
> +                           const unsigned char *cp, const char *fp, int =
count)
> +{
...
> +               if (fp && *fp++) {
> +                       netdev_err(elm->dev, "Error in received character=
 stream. Check your wiring.");
> +
> +                       elm327_hw_failure(elm);
> +
> +                       goto out;
> +               }
> +
> +               /* Ignore NUL characters, which the PIC microcontroller m=
ay
> +                * inadvertently insert due to a known hardware bug.
> +                * See ELM327 documentation, which refers to a Microchip =
PIC
> +                * bug description.
> +                */
> +               if (*cp !=3D 0) {
...
> +
> +       if (count >=3D 0) {
> +               netdev_err(elm->dev, "Receive buffer overflowed. Bad chip=
 or wiring?");
> +
> +               elm327_hw_failure(elm);
> +
...
> +       }
> +
> +       actual =3D elm->tty->ops->write(elm->tty, elm->txhead, elm->txlef=
t);
> +       if (actual < 0) {
> +               netdev_err(elm->dev,
> +                          "Failed to write to tty %s.\n",
