...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 20A7C47DFE9
	for <lists+linux-kernel@lfdr.de>; Thu, 23 Dec 2021 08:50:21 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1347608AbhLWHti (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Thu, 23 Dec 2021 02:49:38 -0500
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:41156 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1347388AbhLWHsS (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Thu, 23 Dec 2021 02:48:18 -0500
Received: from mail-yb1-xb4a.google.com (mail-yb1-xb4a.google.com [IPv6:2607:f8b0:4864:20::b4a])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 40EFDC0698DC
        for <linux-kernel@vger.kernel.org>; Wed, 22 Dec 2021 23:48:11 -0800 (PST)
Received: by mail-yb1-xb4a.google.com with SMTP id g36-20020a25ae64000000b005c1f46f7ee6so8753697ybe.8
        for <linux-kernel@vger.kernel.org>; Wed, 22 Dec 2021 23:48:11 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=google.com; s=20210112;
        h=date:in-reply-to:message-id:mime-version:references:subject:from:to
         :cc;
        bh=CsmehZxdtr4mzWpchiEDLLM8u5lq+Hr6Xrox8uy1DZA=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        bh=CsmehZxdtr4mzWpchiEDLLM8u5lq+Hr6Xrox8uy1DZA=;
        b=
X-Gm-Message-State: AOAM531QOlvCYifttSmQdVBCQzZfMS4PDALd9qT2T8XkVfYOzOA9CtTW
...
X-Received: from irogers.svl.corp.google.com ([2620:15c:2cd:202:4fef:90ff:589d:24ca])
 (user=irogers job=sendgmr) by 2002:a25:820e:: with SMTP id
 q14mr1618749ybk.396.1640245690473; Wed, 22 Dec 2021 23:48:10 -0800 (PST)
Date:   Wed, 22 Dec 2021 23:45:41 -0800
In-Reply-To: <20211223074541.3318938-1-irogers@google.com>
Message-Id: <20211223074541.3318938-49-irogers@google.com>
Mime-Version: 1.0
References: <20211223074541.3318938-1-irogers@google.com>
X-Mailer: git-send-email 2.34.1.307.g9b7440fafd-goog
Subject: [PATCH v2 48/48] perf cpumap: Give CPUs their own type.
...
index 5a65dc7a1580..224030c0ef5f 100644
...
index 0338b813585a..dde884dd9510 100644
...
index de4af7af34c8..aae8a236fa26 100644
--- a/tools/perf/builtin-script.c
+++ b/tools/perf/builtin-script.c
@@ -2113,7 +2113,8 @@ static struct scripting_ops	*scripting_ops;
...
@@ -2132,7 +2133,7 @@ static void __process_stat(struct evsel *counter, u64 tstamp)
...
index a535fd360d46..d644eaa7cb87 100644
...
+	struct perf_cpu cpu;
 	...
 	if (!c)
@@ -198,14 +199,14 @@ struct cpu_aggr_map *cpu_aggr_map__new(const struct perf_cpu_map *cpus,
...
+struct aggr_cpu_id aggr_cpu_id__die(struct perf_cpu cpu, void *data)
 {
 	...
 	int die;
@@ -228,13 +229,13 @@ struct aggr_cpu_id aggr_cpu_id__die(int cpu, void *data)
 	return id;
 }
...
@@ -253,7 +254,7 @@ struct aggr_cpu_id aggr_cpu_id__core(int cpu, void *data)
...
@@ -267,7 +268,7 @@ struct aggr_cpu_id aggr_cpu_id__cpu(int cpu, void *data)
...
@@ -315,8 +316,8 @@ static void set_max_cpu_num(void)
 	int ret = -1;
...
+	max_present_cpu_num.cpu = 4096;
 	...
 	if (!mnt)
@@ -329,7 +330,7 @@ static void set_max_cpu_num(void)
 		goto out;
 	}
...
@@ -340,11 +341,11 @@ static void set_max_cpu_num(void)
 		goto out;
 	}
...
@@ -383,31 +384,31 @@ int cpu__max_node(void)
 	return max_node_num;
 }
...
@@ -417,13 +418,13 @@ static int init_cpunode_map(void)
 	set_max_cpu_num();
 	set_max_node_num();
...
+	for (i = 0; i < max_cpu_num.cpu; i++)
 	...
 	return 0;
@@ -484,35 +485,37 @@ int cpu__setup_cpunode_map(void)
...
+						map->map[start].cpu, map->map[end].cpu);
 			...
 			first = false;
 			start = i;
@@ -539,23 +542,23 @@ size_t cpu_map__snprint_mask(struct perf_cpu_map *map, char *buf, size_t size)
 	int i, cpu;
 	char *ptr = buf;
...
+	for (cpu = last_cpu.cpu / 4 * 4; cpu >= 0; cpu -= 4) {
 		...
 		if (cpu % 8)
@@ -591,7 +594,7 @@ bool aggr_cpu_id__equal(const struct aggr_cpu_id *a, const struct aggr_cpu_id *b
 		a->socket == b->socket &&
 		a->die == b->die &&
 		a->core == b->core &&
-		a->cpu == b->cpu;
...
@@ -601,7 +604,7 @@ bool aggr_cpu_id__is_empty(const struct aggr_cpu_id *a)
 		a->socket == -1 &&
 		a->die == -1 &&
 		a->core == -1 &&
-		a->cpu == -1;
...
@@ -612,7 +615,7 @@ struct aggr_cpu_id aggr_cpu_id__empty(void)
 		.socket = -1,
 		.die = -1,
 		.core = -1,
-		.cpu = -1
...
index 4b510a1f27d4..577ebcb67fe1 100644
...
index c0c010350bc2..6af4ec718852 100644
--- a/tools/perf/util/scripting-engines/trace-event-python.c
+++ b/tools/perf/util/scripting-engines/trace-event-python.c
@@ -1553,7 +1553,7 @@ static void get_handler_name(char *str, size_t size,
 }
...
+process_stat(struct evsel *counter, struct perf_cpu cpu, int thread, u64 tstamp,
 ...
 {
 	PyObject *handler, *t;
@@ -1573,7 +1573,7 @@ process_stat(struct evsel *counter, int cpu, int thread, u64 tstamp,
 		return;
 	}
...
@@ -1597,7 +1597,7 @@ static void python_process_stat(struct perf_stat_config *config,
 	int cpu, thread;
...
index d8857d1b6d7c..eb2c18a8ce02 100644
--- a/tools/perf/util/session.c
+++ b/tools/perf/util/session.c
@@ -2537,15 +2537,15 @@ int perf_session__cpu_bitmap(struct perf_session *session,
 	}
...
+		set_bit(cpu.cpu, cpu_bitmap);
 	...
 	err = 0;
@@ -2597,7 +2597,7 @@ int perf_event__process_id_index(struct perf_session *session,
 		if (!sid)
 			return -ENOENT;
 		sid->idx = e->idx;
-		sid->cpu = e->cpu;
...
index 81a288a7ae46..066231f10b06 100644
