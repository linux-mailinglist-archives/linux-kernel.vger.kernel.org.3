...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 01C604AA5EB
	for <lists+linux-kernel@lfdr.de>; Sat,  5 Feb 2022 03:35:12 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1379165AbiBECfB (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Fri, 4 Feb 2022 21:35:01 -0500
Received: from linux.microsoft.com ([13.77.154.182]:45084 "EHLO
        linux.microsoft.com" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1379011AbiBECeZ (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Fri, 4 Feb 2022 21:34:25 -0500
Received: from IOURIT-Z4.ntdev.corp.microsoft.com (unknown [192.182.151.181])
        by linux.microsoft.com (Postfix) with ESMTPSA id 1C6C320B83DE;
        Fri,  4 Feb 2022 18:34:25 -0800 (PST)
DKIM-Filter: OpenDKIM Filter v2.11.0 linux.microsoft.com 1C6C320B83DE
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=linux.microsoft.com;
        s=default; t=1644028465;
        bh=8BHGH5p78TXG0DKQOUPapuXC2TvlZuX5xmDKMWPHU6A=;
        h=From:To:Cc:Subject:Date:In-Reply-To:References:From;
        b=VYUFLBYaiAcf1jba/0RXPoiTRLjcYL6gHIvhkOxL1ApxA1r4GYPDlhqQ4WMYSBRJz
         c6/r2zdlNBNHbhNY3ZzWiAE42qg1c7TQXDo8Zv4yudV5TI2qGbvCVU2UJZFJGveZEC
         OIqDpZqkQ78Husw1mFc/Znl1VhBSXy87o739nLu8=
From:   Iouri Tarassov <iourit@linux.microsoft.com>
To:     kys@microsoft.com, haiyangz@microsoft.com, sthemmin@microsoft.com,
        wei.liu@kernel.org, linux-hyperv@vger.kernel.org
Cc:     linux-kernel@vger.kernel.org, spronovo@microsoft.com,
        gregkh@linuxfoundation.org
Subject: [PATCH v2 10/24] drivers: hv: dxgkrnl: Sharing of sync objects
Date:   Fri,  4 Feb 2022 18:34:08 -0800
Message-Id: <7ed5233e738f34e884a136534460bac00cc536b5.1644025661.git.iourit@linux.microsoft.com>
...
Implement creation of a shared sync object and IOCTLs for sharing
dxgsyncobject objects between processes in the virtual machine.

Sync objects are shared using FD (file descriptor) handles.
...
To use a shared sync object in WDDM API calls, the object needs to be opened
using its FD handle. A sync object is opened by the following ioctl:
...
index 1b8f2b63eb6d..e32b4bc16ab8 100644
...
+
 ...
 {
@@ -952,6 +972,63 @@ void dxgprocess_adapter_remove_device(struct dxgdevice *device)
...
@@ -1086,6 +1163,12 @@ void dxgsyncobject_release(struct kref *refcount)
 	struct dxgsyncobject *syncobj;
...
index f3ea98f0b384..662fa27fd1ed 100644
...
+struct dxgsharedsyncobject;
 ...
 #include "misc.h"
 #include "hmgr.h"
@@ -118,6 +119,18 @@ struct dxghosteventcpu {
...
@@ -127,6 +140,8 @@ struct dxgsyncobject {
...
+	struct list_head		shared_syncobj_list_entry;
 	...
 	struct dxgadapter		*adapter;
 	/*
@@ -137,6 +152,8 @@ struct dxgsyncobject {
...
@@ -168,6 +185,41 @@ struct dxgvgpuchannel {
...
@@ -385,6 +439,10 @@ void dxgadapter_release_lock_shared(struct dxgadapter *adapter);
...
@@ -697,6 +755,11 @@ int dxgvmb_send_wait_sync_object_cpu(struct dxgprocess *process,
...
index 3af5a7a3ac74..3928512bcc64 100644
...
index af8d1aab871f..659afb59b4fd 100644
--- a/drivers/hv/dxgkrnl/dxgvmbus.c
+++ b/drivers/hv/dxgkrnl/dxgvmbus.c
@@ -694,6 +694,69 @@ int dxgvmb_send_destroy_process(struct d3dkmthandle process)
 	return ret;
 }
