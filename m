...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 06FDF4756F5
	for <lists+linux-kernel@lfdr.de>; Wed, 15 Dec 2021 11:54:08 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S241802AbhLOKyA (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Wed, 15 Dec 2021 05:54:00 -0500
Received: from foss.arm.com ([217.140.110.172]:48414 "EHLO foss.arm.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S236099AbhLOKx7 (ORCPT <rfc822;linux-kernel@vger.kernel.org>);
        Wed, 15 Dec 2021 05:53:59 -0500
Received: from usa-sjc-imap-foss1.foss.arm.com (unknown [10.121.207.14])
        by usa-sjc-mx-foss1.foss.arm.com (Postfix) with ESMTP id C6B7AD6E;
        Wed, 15 Dec 2021 02:53:58 -0800 (PST)
Received: from FVFF77S0Q05N (unknown [172.31.20.19])
        by usa-sjc-imap-foss1.foss.arm.com (Postfix) with ESMTPSA id 97B223F774;
        Wed, 15 Dec 2021 02:53:57 -0800 (PST)
Date:   Wed, 15 Dec 2021 10:53:51 +0000
From:   Mark Rutland <mark.rutland@arm.com>
To:     tjcw@cantab.net
Cc:     linux-kernel@vger.kernel.org, Chris Ward <tjcw01@gmail.com>,
        linux-arm-kernel@lists.infradead.org
Subject: Re: ARM64 kernel crash
Message-ID: <YbnCbyPdsRNFb1iz@FVFF77S0Q05N>
References: <CAC=wTOhV_QTAbH+u8amNF4+wu6ae5qCLmtWW4Z2zNdVZHrfA=Q@mail.gmail.com>
MIME-Version: 1.0
Content-Type: text/plain; charset=utf-8
Content-Disposition: inline
Content-Transfer-Encoding: 8bit
In-Reply-To: <CAC=wTOhV_QTAbH+u8amNF4+wu6ae5qCLmtWW4Z2zNdVZHrfA=Q@mail.gmail.com>
Precedence: bulk
...
Are you able to test with a mainline kernel? e.g. a baseline v5.4.0, or current
longterm stable 5.4.165?

Upstream maintainers aren't really able to help with downstream vendor kernels;
if you can reproduce this with mainline we're better placed to help, and if you
try but cannot reproduce the issue with mainline that might indicate that the
issue is with the vendor kernel.

Presuambly Canonical have the same problem here -- they cannot help with a
kernel they do not maintain.
...
This critically depends on what they've changed within the kernel in their
vendor tree. That said, from the below this doesn't seem to be an
"incompatibility" so much as a kernel bug.

> 2) Is there a known problem with this kernel level on ARM64 hardware ?

I am not aware of a specific issue in this area, though people are mostly
testing newer kernels than this now.

Have you tried searching the lore mailing list archives for anything similar,
e.g.

  https://lore.kernel.org/lkml/?q=Unable+to+handle+kernel+NULL+pointer+dereference
  https://lore.kernel.org/linux-arm-kernel/?q=Unable+to+handle+kernel+NULL+pointer+dereference
  
  https://lore.kernel.org/lkml/?q=%22Unable+to+handle+kernel+NULL+pointer+dereference%22+%22__wake_up_common%22
  https://lore.kernel.org/linux-arm-kernel/?q=%22Unable+to+handle+kernel+NULL+pointer+dereference%22+%22__wake_up_common%22

> 3) Would it be likely to be productive to move to a newer Xilinx kernel ?

As above, using a mainline kernel would be ideal for testing.
...
> ZynqMP> setenv bootargs "console=ttyPS0,115200n8 root=/dev/nfs
> rootfstype=nfs nfsroot=${serverip}:/export/"
...
>          2.5 MiB/s
> done
> Bytes transferred = 8478220 (815e0c hex)
...
>      Data Size:    8444173 Bytes = 8.1 MiB
...
>      Data Size:    32155 Bytes = 31.4 KiB
>      Architecture: AArch64
>      Hash algo:    sha256
>      Hash value:
...
> [    0.000000] Built 1 zonelists, mobility grouping on.  Total pages: 4128516
> [    0.000000] Kernel command line: console=ttyPS0,115200n8
> root=/dev/nfs rootfstype=nfs nfsroot=9.2.225.84
...
> start_kernel+0x2a8/0x42c with crng_init=0
...
> [    0.008980] futex hash table entries: 1024 (order: 4, 65536 bytes, linear)
...
> [    0.134111] audit: type=2000 audit(0.132:1): state=initialized
...
> [    2.332236] VFS: Dquot-cache hash table entries: 512 (order 0, 4096 bytes)
...
> [    2.368951] workingset: timestamp_bits=46 max_order=22 bucket_order=0
...
> [    2.369821] jffs2: version 2.2. (NAND) (SUMMARY)  © 2001-2006 Red Hat, Inc.
...
> [    2.383788] xilinx-xdma-pcie a0000000.axi-pcie: Unable to find misc IRQ line
...
> [    2.422943] mtdoops: mtd device (mtddev=name/number) must be supplied
...
> [    2.461340] can: netlink gateway (rev 20190810) max_hops=1
...
> [    2.463025] Btrfs loaded, crc32c=crc32c-generic
> [    2.472375] ff000000.serial: ttyPS0 at MMIO 0xff000000 (irq = 38,
...
> [    3.798608] xilinx-zynqmp-dma fd500000.dma: ZynqMP DMA driver Probe success
> [    3.805716] xilinx-zynqmp-dma fd510000.dma: ZynqMP DMA driver Probe success
> [    3.812828] xilinx-zynqmp-dma fd520000.dma: ZynqMP DMA driver Probe success
> [    3.819943] xilinx-zynqmp-dma fd530000.dma: ZynqMP DMA driver Probe success
> [    3.827054] xilinx-zynqmp-dma fd540000.dma: ZynqMP DMA driver Probe success
> [    3.834158] xilinx-zynqmp-dma fd550000.dma: ZynqMP DMA driver Probe success
> [    3.841265] xilinx-zynqmp-dma fd560000.dma: ZynqMP DMA driver Probe success
> [    3.848377] xilinx-zynqmp-dma fd570000.dma: ZynqMP DMA driver Probe success
> [    3.855552] xilinx-zynqmp-dma ffa80000.dma: ZynqMP DMA driver Probe success
> [    3.862660] xilinx-zynqmp-dma ffa90000.dma: ZynqMP DMA driver Probe success
> [    3.869764] xilinx-zynqmp-dma ffaa0000.dma: ZynqMP DMA driver Probe success
> [    3.876871] xilinx-zynqmp-dma ffab0000.dma: ZynqMP DMA driver Probe success
> [    3.883982] xilinx-zynqmp-dma ffac0000.dma: ZynqMP DMA driver Probe success
> [    3.891085] xilinx-zynqmp-dma ffad0000.dma: ZynqMP DMA driver Probe success
> [    3.898192] xilinx-zynqmp-dma ffae0000.dma: ZynqMP DMA driver Probe success
> [    3.905308] xilinx-zynqmp-dma ffaf0000.dma: ZynqMP DMA driver Probe success
...
> /amba/spi@ff0f0000/flash@0
> [    3.928451] macb ff0e0000.ethernet: Not enabling partial store and forward
> [    3.935888] libphy: MACB_mii_bus: probed
> [    3.973440] zynqmp_pll_disable() clock disable failed for dpll_int, ret = -13
...
> [    8.259316]      device=eth0, hwaddr=00:0a:35:00:22:01,
> ipaddr=9.2.227.19, mask=255.255.248.0, gw=9.2.22
> [    8.269052]      host=9.2.227.19, domain=, nis-domain=(none)
> [    8.274701]      bootserver=9.2.225.84, rootserver=9.2.225.84, rootpath=
...
> [FAILED] Failed to start Postfix Ma… Transport Agent (instance -).
...
Figuring out which pointer is NULL here would be helpful. If you have the vmlinux to hand, you can run:

  ./scripts/faddr2line vmlinux __wake_up_common+0x58/0x170

... and that should tell you which line the dereference was on, which should
hopefully imply which pointer was NULL, and you can work backwards from that.

> [  112.915231] BUG: Bad page map in process dd  pte:20000b5600efd3 pmd:b68cda003

This means that something has gone very wrong with the page tables, and for
example that could be caused by any of:

* The kernel page table code is dodgy. If so, I'd suspect this could be
  reproduced with a mainline kernel, unless a downstream kernel modification
  has caused the issue by corrupting some mm data structure.

* Something (e.g. a driver or a device) is corrupting memory arbitrarily.
  Depending on what specifically, that could be an issue with upstream, or
  something introduced in a downstream kernel.

* Memory is physically dodgy, or the memory map described to the kernel by FW
  is wrong. It would be worth booting with `memtest` on the kernel command line
  (assuming your kernel was built with CONFIG_MEMTEST), which will make the
  kernel check that memory works as expected early in the boot process, and
  would help to rule this out.

The remaining issues below look similar; other NULL dereferences and dodgy page
tables, and the same rationale and advice applies there.
...
> [  113.000066]   ESR = 0x96000004
> [  113.000068]   EC = 0x25: DABT (current EL), IL = 32 bits
> [  113.000070]   SET = 0, FnV = 0
> [  113.000071]   EA = 0, S1PTW = 0
> [  113.000072] Data abort info:
> [  113.000074]   ISV = 0, ISS = 0x00000004
> [  113.000075]   CM = 0, WnR = 0
> [  113.000078] user pgtable: 4k pages, 48-bit VAs, pgdp=0000000b6502b000
> [  113.000080] [0000000000000020] pgd=0000000000000000
...
> [  113.000236]   ESR = 0x96000004
> [  113.000238]   EC = 0x25: DABT (current EL), IL = 32 bits
> [  113.000240]   SET = 0, FnV = 0
> [  113.000241]   EA = 0, S1PTW = 0
> [  113.000242] Data abort info:
> [  113.000244]   ISV = 0, ISS = 0x00000004
> [  113.000245]   CM = 0, WnR = 0
> [  113.000248] user pgtable: 4k pages, 48-bit VAs, pgdp=0000000b6502b000
> [  113.000250] [00000005aeed7d90] pgd=0000000000000000
...
> [  113.003180] user pgtable: 4k pages, 48-bit VAs, pgdp=0000000b69ff9000
> [  113.006475]  brd_make_request+0x1c8/0x1d0
> [  113.009429] [0000000000000020] pgd=0000000000000000
...
> [  113.166209] BUG: Bad page map in process dd  pte:20000b5275cfd3 pmd:b68cda003
...
>           5.4.0-xilinx-v2020.2 #1
> [  114.050322] BUG: Bad page map in process dd  pte:20000b5552ffd3 pmd:b68cda003
...
>           5.4.0-xilinx-v2020.2 #1
> [  114.402022] BUG: Bad page map in process dd  pte:20000b51bd4fd3 pmd:b68cda003
...
>           5.4.0-xilinx-v2020.2 #1
> [  114.848857] BUG: Bad page map in process dd  pte:20000b50cdcfd3 pmd:b68cda003
...
> [  115.010936]   ESR = 0x96000004
> [  115.010938]   EC = 0x25: DABT (current EL), IL = 32 bits
> [  115.010940]   SET = 0, FnV = 0
> [  115.010942]   EA = 0, S1PTW = 0
> [  115.010943] Data abort info:
> [  115.010944]   ISV = 0, ISS = 0x00000004
> [  115.010946]   CM = 0, WnR = 0
> [  115.010948] user pgtable: 4k pages, 48-bit VAs, pgdp=0000000b66d8d000
> [  115.010950] [0000000000000020] pgd=0000000000000000
