...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id AEDCE550E83
	for <lists+linux-kernel@lfdr.de>; Mon, 20 Jun 2022 04:12:01 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S230024AbiFTCLz (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Sun, 19 Jun 2022 22:11:55 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:55976 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S229774AbiFTCLx (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Sun, 19 Jun 2022 22:11:53 -0400
Received: from mail-pj1-x102d.google.com (mail-pj1-x102d.google.com [IPv6:2607:f8b0:4864:20::102d])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 748AAB871
        for <linux-kernel@vger.kernel.org>; Sun, 19 Jun 2022 19:11:52 -0700 (PDT)
Received: by mail-pj1-x102d.google.com with SMTP id go6so4108118pjb.0
        for <linux-kernel@vger.kernel.org>; Sun, 19 Jun 2022 19:11:52 -0700 (PDT)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        d=gmail.com; s=20210112;
        h=from:to:cc:subject:date:message-id:mime-version
         :content-transfer-encoding;
        bh=hhN9Eeyd98g/WZSwmGVpVtmeYBzF+xaNE9o0DjrSVoY=;
        b=
X-Google-DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed;
        ...
        h=x-gm-message-state:from:to:cc:subject:date:message-id:mime-version
         :content-transfer-encoding;
        bh=hhN9Eeyd98g/WZSwmGVpVtmeYBzF+xaNE9o0DjrSVoY=;
        b=
X-Gm-Message-State: AJIora81+HoOR+vW4IoDe/ELJkNCWnDnq0Wm8320pde93oj2sqx2LeY3
        5G1+iJCPq/PrMD/XOnperP5yMOV3Oco=
X-Google-Smtp-Source: AGRyM1vqV9YPfyW2ak9jAy+RGRSOQ6R+NF8PO2z7k/cplOIPjik/QQEKFtW4XN/iSdIRJ+pzbKDOdA==
X-Received: by 2002:a17:903:2303:b0:166:313f:a85f with SMTP id d3-20020a170903230300b00166313fa85fmr21865669plh.57.1655691111749;
        Sun, 19 Jun 2022 19:11:51 -0700 (PDT)
Received: from localhost.localdomain ([193.203.214.57])
        by smtp.gmail.com with ESMTPSA id a30-20020a62d41e000000b00518c8397e7esm7580157pfh.211.2022.06.19.19.11.50
...
Subject: [PATCH] [RFC patch] psi: introduce memory.pressure.stat
Date:   Mon, 20 Jun 2022 02:11:46 +0000
Message-Id: <20220620021146.936566-1-ran.xiaokai@zte.com.cn>
...
Psi memory pressure account for all the mem stall in the system level.
And didnot provide a detailed information why the stall happen.
This patch introduce a cgroupu knob memory.pressure.stat, it
tells the detailed stall information of all memory events and it
...
@@ -41,7 +42,7 @@ __poll_t psi_trigger_poll(void **trigger_ptr, struct
file *file,
 
 static inline void psi_init(void) {}
 
-static inline void psi_memstall_enter(unsigned long *flags) {}
+static inline void psi_memstall_enter(unsigned long *flags, int
mem_state) {}
...
+#define TSK_MEMSTALL_KSWAPD 	(1 << (PSI_MEM_KSWAPD +
TSK_MEMSTALL_SHIFT))
+#define TSK_MEMSTALL_DRECLAIM 	(1 << (PSI_MEM_KCOMPACTED +
TSK_MEMSTALL_SHIFT))
+#define TSK_MEMSTALL_KCOMPACTED 	(1 << (PSI_MEM_DCOMPACT +
TSK_MEMSTALL_SHIFT))
+#define TSK_MEMSTALL_DCOMPACT 	(1 << (PSI_MEM_CGROUP +
TSK_MEMSTALL_SHIFT))
+#define TSK_MEMSTALL_CGROUP 	(1 << (PSI_MEM_DRECALAIM +
TSK_MEMSTALL_SHIFT))
+#define TSK_MEMSTALL_WORKINGSET 		(1 <<
(PSI_MEM_WORKINGSET + TSK_MEMSTALL_SHIFT))
+#define TSK_MEMSTALL_MASK	(PSI_MASK(TSK_MEMSTALL_SHIFT) <<
TSK_COUNT_SHIFT)
+
...
@@ -3613,6 +3613,13 @@ static int cgroup_memory_pressure_show(struct
seq_file *seq, void *v)
 
 	return psi_show(seq, psi, PSI_MEM);
 }
+static int cgroup_memory_pressure_stat_show(struct seq_file *seq, void
*v)
+{
+	struct cgroup *cgroup = seq_css(seq)->cgroup;
+	struct psi_group *psi = cgroup->id == 1 ? &psi_system :
&cgroup->psi;
+
...
@@ -279,6 +279,35 @@ static void get_recent_times(struct psi_group
*group, int cpu,
 	}
 }
 
+static void get_recent_mem_times(struct psi_group *group, int cpu, u32
*times_mem)
+{
...
+		now = cpu_clock(cpu);
+		memcpy(times_mem, groupc->times_mem,
sizeof(groupc->times_mem));
...
+
 static void calc_avgs(unsigned long avg[3], int missed_periods,
 		      u64 time, u64 period)
 {
@@ -304,6 +333,7 @@ static void collect_percpu_times(struct psi_group
*group,
 				 u32 *pchanged_states)
 {
...
+	u64 delta_mems[PSI_MEM_STATES - 1] = { 0, };
 	...
 	int cpu;
@@ -319,11 +349,16 @@ static void collect_percpu_times(struct psi_group
*group,
 	 */
...
@@ -974,6 +1030,53 @@ int psi_show(struct seq_file *m, struct psi_group
...
+
+		total = div_u64(group->total_mems[PSI_AVGS],
NSEC_PER_USEC);
+
...
@@ -998,7 +1101,10 @@ static int psi_memory_open(struct inode *inode,
struct file *file)
 {
...
@@ -1271,7 +1377,12 @@ static const struct file_operations
psi_memory_fops = {
 	.poll           = psi_fop_poll,
...
+	proc_create("pressure/memory_stat", 0, NULL,
