...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id 053905965CC
	for <lists+linux-kernel@lfdr.de>; Wed, 17 Aug 2022 01:02:30 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S237893AbiHPXBj (ORCPT <rfc822;lists+linux-kernel@lfdr.de>);
        Tue, 16 Aug 2022 19:01:39 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:59978 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S237805AbiHPXBO (ORCPT
        <rfc822;linux-kernel@vger.kernel.org>);
        Tue, 16 Aug 2022 19:01:14 -0400
Received: from mellanox.co.il (mail-il-dmz.mellanox.com [193.47.165.129])
        by lindbergh.monkeyblade.net (Postfix) with ESMTP id 5E2A67C322
        for <linux-kernel@vger.kernel.org>; Tue, 16 Aug 2022 16:01:10 -0700 (PDT)
Received: from Internal Mail-Server by MTLPINE1 (envelope-from asmaa@mellanox.com)
        with SMTP; 17 Aug 2022 01:54:26 +0300
Received: from bu-vnc02.mtbu.labs.mlnx (bu-vnc02.mtbu.labs.mlnx [10.15.2.65])
        by mtbu-labmailer.labs.mlnx (8.14.4/8.14.4) with ESMTP id 27GMsPhj022548;
        Tue, 16 Aug 2022 18:54:25 -0400
Received: (from asmaa@localhost)
        by bu-vnc02.mtbu.labs.mlnx (8.14.7/8.13.8/Submit) id 27GMsPsE009262;
        Tue, 16 Aug 2022 18:54:25 -0400
...
Subject: [PATCH v1 6/7] i2c-mlxbf.c: support BlueField-3 SoC
Date:   Tue, 16 Aug 2022 18:54:11 -0400
Message-Id: <20220816225412.9095-7-asmaa@nvidia.com>
...
index c6959ba7113e..cc87c2511678 100644
...
@@ -227,14 +224,14 @@ static u64 mlxbf_i2c_corepll_frequency;
...
@@ -261,18 +258,18 @@ static u64 mlxbf_i2c_corepll_frequency;
...
@@ -285,13 +282,13 @@ static u64 mlxbf_i2c_corepll_frequency;
...
+#define MLXBF_I2C_SMBUS_SLAVE_ADDR_EN_BIT     BIT(7)
 ...
 /*
@@ -300,6 +297,58 @@ static u64 mlxbf_i2c_corepll_frequency;
  */
...
@@ -319,27 +368,12 @@ struct mlxbf_i2c_timings {
...
-
 struct mlxbf_i2c_smbus_request {
 	u8 slave;
 	u8 operation_cnt;
@@ -353,24 +387,36 @@ struct mlxbf_i2c_resource {
 	u8 type;
 };
...
@@ -384,6 +430,10 @@ static struct resource mlxbf_i2c_corepll_yu_params =
...
@@ -393,34 +443,6 @@ static struct mutex mlxbf_i2c_coalesce_lock;
...
@@ -440,6 +462,11 @@ static struct mlxbf_i2c_resource mlxbf_i2c_corepll_res[] = {
...
@@ -456,24 +483,13 @@ static u8 mlxbf_i2c_bus_count;
...
+static u32 mlxbf_i2c_poll(void __iomem *io, u32 addr, u32 mask,
 ...
 {
 	u32 bits;
@@ -495,13 +511,13 @@ static u32 mlxbf_smbus_poll(void __iomem *io, u32 addr, u32 mask,
...
+	if (mlxbf_i2c_poll(priv->mst->io, addr, mask, true, timeout))
 	...
 	return false;
@@ -544,7 +560,7 @@ static int mlxbf_i2c_smbus_check_status(struct mlxbf_i2c_priv *priv)
...
@@ -557,7 +573,7 @@ static int mlxbf_i2c_smbus_check_status(struct mlxbf_i2c_priv *priv)
...
@@ -582,7 +598,8 @@ static int mlxbf_i2c_smbus_check_status(struct mlxbf_i2c_priv *priv)
 }
...
+				       bool is_master)
 {
 	...
 	u32 data32;
@@ -599,12 +616,16 @@ static void mlxbf_i2c_smbus_write_data(struct mlxbf_i2c_priv *priv,
 	 */
...
+				      bool is_master)
 {
 	u32 data32, mask;
 	u8 byte, offset;
@@ -620,14 +641,20 @@ static void mlxbf_i2c_smbus_read_data(struct mlxbf_i2c_priv *priv,
 	 */
...
@@ -653,16 +680,16 @@ static int mlxbf_i2c_smbus_enable(struct mlxbf_i2c_priv *priv, u8 slave,
...
@@ -699,7 +726,7 @@ mlxbf_i2c_smbus_start_transaction(struct mlxbf_i2c_priv *priv,
 	addr = slave << 1;
...
+	if (WARN_ON(!mlxbf_i2c_smbus_master_wait_for_idle(priv)))
 	...
 	/* Set first byte. */
@@ -752,7 +779,7 @@ mlxbf_i2c_smbus_start_transaction(struct mlxbf_i2c_priv *priv,
...
@@ -764,13 +791,13 @@ mlxbf_i2c_smbus_start_transaction(struct mlxbf_i2c_priv *priv,
 	if (read_en) {
...
@@ -782,7 +809,7 @@ mlxbf_i2c_smbus_start_transaction(struct mlxbf_i2c_priv *priv,
...
+			priv->mst->io + priv->chip->smbus_master_fsm_off);
 	...
 	return ret;
@@ -1071,7 +1098,7 @@ static void mlxbf_i2c_set_timings(struct mlxbf_i2c_priv *priv,
...
@@ -1082,34 +1109,34 @@ static void mlxbf_i2c_set_timings(struct mlxbf_i2c_priv *priv,
...
@@ -1521,7 +1548,7 @@ static int mlxbf_i2c_slave_enable(struct mlxbf_i2c_priv *priv,
...
@@ -1543,7 +1570,7 @@ static int mlxbf_i2c_slave_enable(struct mlxbf_i2c_priv *priv,
...
@@ -1578,7 +1605,7 @@ static int mlxbf_i2c_slave_disable(struct mlxbf_i2c_priv *priv, u8 addr)
...
@@ -1599,7 +1626,7 @@ static int mlxbf_i2c_slave_disable(struct mlxbf_i2c_priv *priv, u8 addr)
 			if (addr_tmp == addr) {
...
@@ -1703,7 +1730,7 @@ static int mlxbf_i2c_init_slave(struct platform_device *pdev,
 	int ret;
...
@@ -1718,7 +1745,7 @@ static int mlxbf_i2c_init_slave(struct platform_device *pdev,
...
@@ -1763,13 +1790,13 @@ static bool mlxbf_i2c_has_coalesce(struct mlxbf_i2c_priv *priv, bool *read,
 	return true;
 }
...
+	if (mlxbf_i2c_poll(priv->slv_cause->io, addr, mask, false, timeout))
 	...
 	return false;
@@ -1808,7 +1835,7 @@ static int mlxbf_i2c_irq_send(struct mlxbf_i2c_priv *priv, u8 recv_bytes)
...
@@ -1862,7 +1889,7 @@ static int mlxbf_i2c_irq_send(struct mlxbf_i2c_priv *priv, u8 recv_bytes)
...
@@ -1871,19 +1898,19 @@ static int mlxbf_i2c_irq_send(struct mlxbf_i2c_priv *priv, u8 recv_bytes)
...
@@ -1901,7 +1928,7 @@ static int mlxbf_i2c_irq_recv(struct mlxbf_i2c_priv *priv, u8 recv_bytes)
...
+				  MLXBF_I2C_SLAVE_DATA_DESC_ADDR, false);
 	...
 	/*
@@ -1937,9 +1964,9 @@ static int mlxbf_i2c_irq_recv(struct mlxbf_i2c_priv *priv, u8 recv_bytes)
...
@@ -1974,7 +2001,7 @@ static irqreturn_t mlxbf_i2c_irq(int irq, void *ptr)
...
@@ -2139,14 +2166,27 @@ static struct mlxbf_i2c_chip_info mlxbf_i2c_chip[] = {
...
@@ -2171,6 +2211,10 @@ static const struct of_device_id mlxbf_i2c_dt_ids[] = {
 		.compatible = "mellanox,i2c-mlxbf2",
...
@@ -2180,6 +2224,7 @@ MODULE_DEVICE_TABLE(of, mlxbf_i2c_dt_ids);
...
@@ -2268,10 +2313,24 @@ static int mlxbf_i2c_probe(struct platform_device *pdev)
 	if (ret < 0)
 		return ret;
...
@@ -2360,7 +2419,13 @@ static int mlxbf_i2c_remove(struct platform_device *pdev)
